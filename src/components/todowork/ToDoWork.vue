<template>
  <div class="1">
    <!-- 安全风险管理待我审批列表-表头-开始 -->
    <div class="biaotou">
      <van-nav-bar title="待我审批" left-text="返回" @click-left="onClickLeft" right-text="关闭" @click-right="onClickRight">
      </van-nav-bar>
...
// 获取代办流程列表数据-拼接url，发送请求获取待办流程列表数据
      GetToDoWorkflowList(done) {
        this.nodataMsg = '加载中...'
        var vm = this;
        var url = this._GLOBAL.API_ROOT + '/GetToDoWorkflowList?baseuserId=' + this.baseuserId + '&pageNo=' + this.pageNo + '&workflowTypeId=' + this.workflowTypeId + '&pageSize=' + this.pageSize + '&workflowId=' + this.workflowId + '&requestName=' + this.requestName
